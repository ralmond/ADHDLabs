...
The upload icon should appear.  (If you don't see the upload icon in the Citrix menu bar, try clicking on the button with the three dots, that will show some additional commands.)
...
### SPSS Command
[F]{.underline}ile \> Import [D]{.underline}ata \> [C]{.underline}SV ...
:::

::: {.callout-tip}
The underlined letters are an accessibility feature built into Windows.  By pressing the `Alt` key and the underlined letter, windows will open the menu, keep pressing the underlined letters to select a menu item.  Thus, the command to open SPSS files can be selected with `Alt+F D C`, and you don't need to use the mouse.  The `...` is another interface convention, it means that selecting this menu item will open a diaglog window for further input.
:::

To read a CSV file, into SPSS version 27 or higher, select `File > Import Data > CSV...` from the menus. (In the documentation for the beta release, IBM gives `File > Import ...`, so they might move this in future versions). This will first open a file picker to let you pick your `.csv` file and then a dialog with a number of options. Check them over to see if you need to change anything, you probably won't. Select `OK` and SPSS will try to read the data file. It will put the results in the spreadsheet like variables view which should now open.
...
::: {.callout-note appearance="simple"}
### SPSS Command
[I]{.underline}nsert > New Te[x]{.underline}t
:::


To practice, select `Intsert > New Text`, and copy and paste the Prevatt et al. (2015) reference into your output. That way, if you ever pick up the output again, you will remember what data are analyzed in this file.

# Editing the Metadata

_Metadata_ is data about the data.  It can include things such as where it was gathered and when.  But it also includes information about what the variable names mean and what kinds of values they can include.  In SPSS much of this is handled through the "Variables" view of the data editor.
...
![Missing Value Dialog](img/MissingValuesBox.png) 
...
To check that we have properly accounted for the missing values, calculate some summaries of the data.  The `Descriptive` command does this.
...
For the nominal and ordinal values, you will want to use a slightly different command, "Frequencies" This time, select only the nominal and ordinal variables (these will have the three circles or three bars icons). You also want APA style tables. The dialog should look like this:
...
There are a number of ways you can compute new varibles in SPSS.  For numeric variables, you can combine them to make new variables with the `Compute Variable` command

::: {.callout-note appearance="simple"}
### SPSS Command
[T]{.underline}ransform > [C]{.underline}ompute Variable ...
:::

This pops up the `Compute Variable` dialog.  To compute a new variable, you need to first supply a name (and possibly a label) for the new variable in the box marked "Target Variable".  Then construct a mathematical expression for computing the new variable, you can select existing variables from the list on the left (select the varaible and then press on the arrow key to move it to the formula), and mathematical functions from the lists on the right.  When you are happy with the expression, press "OK" and a new column should appear in the data view.  

![Calculate a gain score by subtracting two scores](img/ComputeVariable.png)

## Recoding Variables

For nominal and ordinal variables, there are some different functions.
...
::: {.callout-note appearance="simple"}
### SPSS Command
[T]{.underline}ransform > [A]{.underline}utomatic Recode ...
:::

This opens up a dialog box.  Select a variable to recode on the left and press the arrow to move it the the right.  Next, create a new name (it should be different from all other variables you already have) and press new name.  You can recode as many variables as you like.  Press "OK" when you are finished.


![Automatic Recode](img/AutomaticRecode.png)
...
::: {.callout-note appearance="simple"}
### SPSS Command
[T]{.underline}ransform > [R]{.underline}ecode into Different Variables...
:::

This function allows the analyst to collapse different categories. In the example below, categories 1, 4 and 5 are combined into a single new category 1, and 6, 7 and 8 are renumbered to close the gap.  The box on the right shows how the old codes will be translated into new codes.  To add entries, put the old code on the right and the new code on the left and then press "Add" (you can also change or remove mappings that you don't want).  When you are done, press "OK" and SPSS will create a new data column.

![Transformation collapsing categories](img/RecodeDifferentExample.png)

# Getting output from SPSS back to your usual computere.
...
![Selecting Export](img/Export.png) 

You could also open a word document and copy and paste the figures and tables you want to keep into that.

## Printing the variables view

If you want us to check that you have done the data cleaning properties, print out the *Variables* view.  Goto the SPSS data window and select the variables view, and then select `File > Print`.  From the printer list, select a PDF printer, which will then same the output window as a pdf.  If you submit that with your draft, we will check the data cleaning.

## Getting data from a lab computer to your computer.

Chances are you don't have a personal copy of SPSS on your computer, so now you need to get the output files to your personal computer.

### FSU Virtual Lab

Press the 3 circle icon at the top of the screen, and the button with the arrow going down from the cloud to download the files you want.  Select them first on the remote machine and then where you want to put them on your local machine.

The files you leave on the virtual lab will stay around for a while.  It is probably fine to rely on this if you are in a rush and you are coming back later in the week.  I wouldn't leave any data up there that you wouldn't mind using for more than a semester, as they may reset the accounts at any time.

### Lab computers in the LRC (Stone or Tully) or Library

The LRC computers are set to delete all files when you log out. Make sure you have copied you work to a flash drive, or a share drive (e.g., OneDrive or Google Drive), or just email yourself a copy.
...
-   *Methods: Data Description*. This is not your own data collection, but you do need to summarize how @prevatt15 gathered the data. From where did they come? What are the inclusion and exclusion criteria? How big is the sample: how many ADHD and control students are there.
...
```
One extra bit.  If you want to have us check that you have found the missing values and outliers as well as done the labeling, go to variable view and select `File > Print ...` and save the resulting print out and submit it with your draft.  We will let you know if you missed something. 
