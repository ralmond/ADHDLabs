...
However, there is a trick:  _Always use the second line!_  If if the variances are approximately equal, the two lines should be almost the same (the only difference is often a slightly different degrees of freedom).  If the variances are not equal, then the second line is the one you want to use anyway.
...
Also, the SPSS $t$-table has way too many statistics in it, as well as way to many digits.  You are much better off pulling the numbers you need out of the table, and reporting the $t$-test results using APA style.  The recommended way to report this is $t(\langle df \rangle) = \langle t \rangle, p = \langle sig \rangle, d = \langle \text{Cohen's } d \rangle$, where the values in angle brackets are read from the table.  
  * $\langle df \rangle$ is the degrees of freedom.  Round this to one decimal place (or zero if it is large).
  * $\langle t \rangle$ is the Student's $t$ statistic.  The sign is determined solely by whether the mean of Group 1 or Group 2 is bigger.  If it is negative, then it is usually best to drop the sign and make it clear in the text which group had the larger mean.  If you drop the sign on $t$, make sure you drop the sign on $d$ as well.
  * $\langle p \rangle$ this is the $p$-value, which SPSS calls "Sig. (two-tailed)".  If you are doing a one-tailed test, then divide this value by 2.  APA style calls for leaving off the leading zero.  By convention if you see any value smaller than .001 (especially 0, as the $p$-value is almost surely not exactly zero), you should write $p<.001$.  You may also write $p<.01$ or $p<.05$ (if those are true).
  * $\langle d \rangle$ in SPSS 29 and later, there is a Effect Sizes table that has this value (see the next section below).  Note that $t$ and $d$ will always have the same sign, so they should either both be positive or both be negative.
